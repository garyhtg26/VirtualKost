                        ...
                        <tbody class="bg-white">
                            ...
                            @foreach ($kosts as $kost)
                                <tr>
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">{{ $kost->kost_price }}</td>
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                        <a href="{{ route('kosts.show', $kost->id) }}" class="border text-white bg-blue-500 hover:border-blue-500 hover:text-white px-4 py-2 rounded-md">SHOW</a>
                                        <a href="{{ route('kosts.edit', $kost->id) }}" class="border text-white bg-yellow-500 hover:border-yellow-500 hover:text-white px-4 py-2 rounded-md">EDIT</a>
                                        {{-- add delete button using form tag --}}
                                        <form method="post" action="{{ route('kosts.destroy', $kost->id) }}" class="inline">
                                            @csrf
                                            @method('delete')
                                            <button class="border text-white bg-red-500 hover:border-red-500 hover:text-white px-4 py-2 rounded-md">DELETE</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
